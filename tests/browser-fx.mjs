...
try {
  ...
  await page.reload();assert.equal((await saved()).fx.USD,30);assert.equal((await saved()).fx.USDT,30);assert.equal(await page.locator('[name=USDT]').count(),0);assert.equal(await page.locator('[data-app-version]').textContent(),'v0.3.0');await page.locator('[data-action=reload-app]').click();await page.waitForURL('**?_update=*#settings');assert.equal((await saved()).fx.USD,30);
