...
try{
 ...
 for(const [k,v]of [['all','-400'],['tw','+2,000'],['us','-3,000'],['crypto','+600']])assert.equal(await pnl(k).innerText(),v);
 ...
 await page.locator('[data-filter=tw]').click();assert.equal(await pnl('all').innerText(),'-400');
 ...
 await page.setViewportSize({width:390,height:844});await page.locator('[data-filter=all]').click();await page.locator('[data-edit=missing]').click();await page.locator('[name=cost]').fill('40');await page.locator('#record-form button[type=submit]').click();assert.equal(await pnl('all').innerText(),'+200');assert.equal(await page.locator('.profit-incomplete').count(),0);await page.reload();assert.equal(await pnl('all').innerText(),'+200');
 await page.locator('#mobile-nav [data-tab=overview]').click();assert.equal(await pnl('all').innerText(),'+200');await page.locator('.overview-investments [data-tab=holdings]').click();assert.equal(await pnl('us').innerText(),'-2,400');
