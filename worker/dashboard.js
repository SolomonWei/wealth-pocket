// Generated from quotes.mjs for the Cloudflare dashboard editor.
// Public quote gateway. Receives stock symbols only, never portfolio balances or credentials.
const SYMBOL=/^(tw:\d{4,6}[A-Z]?|us:[A-Z][A-Z0-9.\-]{0,14})$/;
function yahooSymbols(key){if(!SYMBOL.test(key))throw Error('Invalid symbol');const [kind,symbol]=key.split(':');return kind==='tw'?[symbol+'.TW',symbol+'.TWO']:[symbol.replaceAll('.','-')];}
function parseYahoo(payload,key,yahooSymbol,now=Date.now()){
...
async function fetchQuote(key,{fetcher=fetch,cache=null,now=Date.now()}={}){
